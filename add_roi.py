...
class AddRoi(QtWidgets.QDialog):
    def __init__(self, parent = None, calib = None, sigma = None, roiCount = 0, monoE = None, monoType = None):
        ...
        if self.Calib is not None and self.Sigma is not None:
            ...
            if monoE is None:
                monoE = min(self.Calib[4095], self.Calib[8191])
            findLimitK = True
            findLimitL3 = True
            findLimitL2 = True
            findLimitM = True
    ...
    def ButtonBox_clicked(self, button):
        if button.text() == "Reset":
            ...
            for row in range(self.CustomROIs.rowCount()):
                ...
                if singleParent:
                    i = tabs.addTab(single.PreviewTab(tabs.parent(), [int(self.CustomROIs.item(row, 4).text()), int(self.CustomROIs.item(row, 6).text())], [int(self.CustomROIs.item(row, 5).text()), int(self.CustomROIs.item(row, 7).text())], float(self.CustomROIs.item(row, 3).text())), self.CustomROIs.item(row, 0).text())
