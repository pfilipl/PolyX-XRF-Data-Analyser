...
class SingleWindow(QtWidgets.QWidget):
    ...
    def loadCalibration(self):
        fileName, _ = QtWidgets.QFileDialog.getOpenFileName(self, "Load calibration", self.MapPath.text(), "Text files(*.txt *.dat);; All files(*)")
        if fileName:
    ...
    def LoadData(self, startLoad = True, importLoad = False):
        ...
        try:
            ...
            load_plots.MapStats2D(self, self.PIN, "PIN", det, "PIN", importLoad = importLoad, Aspect = mapAspect, Cmap = cMap, Norm = norm if self.NormType != "LT" else None)
    ...
    def ReloadData(self):
        ...
        load_plots.MapStats2D(self, self.PIN, "PIN", det, "PIN", Aspect = mapAspect, Cmap = cMap, Norm = norm if self.NormType != "LT" else None)
        ...
        if not resultsPath.is_dir():
            ...
            if outputConfig.exec():
                ...
                if hdf5:
                    ...
                    exec(f'analyse.HDF5(self, self.Data, pathlib.Path(self.MapPath.text()), resultsPath, ROI, str([self.OutputConfig["GenHDF5full"], self.OutputConfig["GenHDF5fullOrigROIs"]]), Calib = self.Calib)')
                self.Progress.setValue(self.Progress.value() + 1)
                QtGui.QGuiApplication.restoreOverrideCursor()
